...
export default function ChartTemperature(props) {
  ...
  const average = Math.round(dataDays.reduce( (acc, elem) => acc + elem['T2m'], 0) / dataDays.length * 100) / 100; 
  const marks = [
    {
      value: 0,
      label: 'H',
      ...
      <div style={{ width: '95%', height: 300 }}>
        <ResponsiveContainer>
          ...
          >
            ...
            <Area type="monotone" dataKey="T2m" stackId="1" stroke="#88b4b8" fill="#88b4b8" />
