...
const dataDays = data.map( (dataElem) => {
    ...
    if( dataElem['time'].split(':')[1] === '2300' ) {
        ...
        time = dataElem['time'].slice(4,6) + '-' + dataElem['time'].slice(6,8);
        return({
            'time': time,
            'T2m': avgT2m,
            'RH': avgRH,
...
// Nouvelle entrée : annee mois jour -> moyenne
// console.log('Average of [' + month + '/' + day + '] - ' + avgDay);
fs.writeFileSync('./pvgis-interesting-data-days.json', JSON.stringify(dataDays));
